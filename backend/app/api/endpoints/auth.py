...
async def get_github_oauth_url():
    ...
    redirect_uri = (
        ...
        or f"{settings.FRONTEND_URL}/oauth/callback?provider=github"
    )
...
async def github_oauth_callback(
    ...
    if not settings.GITHUB_CLIENT_ID or not settings.GITHUB_CLIENT_SECRET:
        raise HTTPException(
            ...
            detail=(
                ...
                "in backend .env. Please configure GitHub OAuth credentials "
                "to proceed."
            ),
        )
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if not access_token:
                logger.error(
                    f"GitHub OAuth error response: {token_data}"
                )
                ...
                raise HTTPException(
                    ...
                    detail=(
                        ...
                        f"{token_data.get('error_description', 'Invalid code or configuration')}."
                    ),
                )
    ...
    if current_user:
        ...
        return Token(
            access_token=token,
            token_type="bearer",
            user=_format_user_response(updated),
        )
...
async def get_google_oauth_url():
    ...
    if not client_id or client_id == "GOOGLE_CLIENT_ID_PLACEHOLDER":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    redirect_uri = (
        ...
        or f"{settings.FRONTEND_URL}/oauth/callback?provider=google"
    )

    state = f"google_{secrets.token_hex(16)}"
...
async def google_oauth_callback(
    ...
    if (
        ...
        raise HTTPException(
            ...
            detail=(
                ...
                "in backend .env. Please configure Google OAuth credentials "
                "to proceed."
            ),
        )
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if not google_access_token:
                logger.error(
                    f"Google OAuth token error: {token_data}"
                )
                ...
                raise HTTPException(
                    ...
                    detail=(
                        ...
                        f"{token_data.get('error_description', 'Invalid code')}."
                    ),
                )
    ...
    # If user is already logged in, connect Google account
    if current_user:
    ...
    # Look up existing user by Google ID or email
    ...
    if user:
        ...
        base_username = (
            email.split("@")[0]
            if email
            else f"user_{google_id[:6]}"
        )

        username = base_username
        counter = 1

        while await UserRepository.get_by_username(
            db,
            username,
        ):
            username = f"{base_username}_{counter}"
...
async def disconnect_github(
    ...
    updated = await UserRepository.disconnect_github(
        db,
        current_user.id,
    )
