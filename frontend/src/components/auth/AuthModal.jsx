...
export default function AuthModal() {
  ...
  const handleGithubOAuth = async () => {
    ...
    try {
      ...
      console.error('GitHub OAuth initiation failed:', err);
      const detail = err.response?.data?.detail || err.message;
      setError(
        typeof detail === 'string'
          ? detail
          : 'Unable to initiate GitHub OAuth. Please ensure backend is running.'
      );
      setOauthLoading(null);
    }
  };
  ...
  const handleGoogleOAuth = async () => {
    ...
    try {
      ...
      setError(
        typeof detail === 'string'
          ? detail
          : 'Unable to initiate Google OAuth. Please ensure backend is running.'
      );
      setOauthLoading(null);
    }
  };
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              className="flex items-center justify-center gap-2 rounded-xl border border-slate-200 dark:border-dark-border bg-white dark:bg-dark-surface px-3 py-2.5 text-xs font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-dark-hover shadow-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {oauthLoading === 'github' ? (
